...
# Place in tiles folder and run "python3 makekml_tile.py"
# Recomended tiles: 1000x1000px at zoom level 12
...
north_tile = int(input("Select north tile: "))
south_tile = int(input("Select south tile: "))
west_tile = int(input("Select west tile (folder name): "))
east_tile = int(input("Select east tile (folder name): "))


sys.stdout = open(filename+".kml",'w')
...
print("<Folder>")
print(f'<name>"{filename},{str(north_tile)},{str(south_tile)},{str(west_tile)},{str(east_tile)}"</name>')
print("<open>0</open>")

folders = os.listdir(str(zoom))
for folder in range(west_tile, east_tile):
    files = os.listdir(str(zoom) + "/" + str(folder))
    for file in range(north_tile, south_tile):
        x = int(folder)
        y = int(file)
        x2 = x + 1
        y2 = y + 1
